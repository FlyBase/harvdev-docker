	 ...
	 if ($#temp >-1 ){
	      ...
	      if ($db_id && defined $AoH_local_id[$level-1]{$hash_level_name{$level-1}}){
                 ...
                 if (defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $db_id){
                    print "\n1.you define two different record with same local_id:$AoH_local_id[$level-1]{$hash_level_name{$level-1}}: at line:$line";
		 }
             ...
             if ($validate_level == $validate_db ) {
                 ...
                 if ($db_id){
	                ...
	                if ($db_id ne '' && defined $AoH_local_id[$level-1]{$hash_level_name{$level-1}}){
                              my $hash_id_key=$hash_level_name{$level-1}.":".$AoH_local_id[$level-1]{$hash_level_name{$level-1}};
                              if (defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $db_id){
                                  print "\n2. you define two different record with same local_id:$AoH_local_id[$level-1]{$hash_level_name{$level-1}}: at line:$line";
		              }
                              $hash_id{$hash_id_key}=$db_id;
	                }
                     }
	                ...
	                if ($db_id ne '' && defined $AoH_local_id[$level-1]{$hash_level_name{$level-1}}){
                              my $hash_id_key=$hash_level_name{$level-1}.":".$AoH_local_id[$level-1]{$hash_level_name{$level-1}};
                              if (defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $db_id){
                                  print "\n3. you define two different record with same local_id:$AoH_local_id[$level-1]{$hash_level_name{$level-1}}: at line:$line";
	 	              }
                              $hash_id{$hash_id_key}=$db_id;
	                }
                     }
	        ...
	        if (!(defined $db_id)){
                    ...
                    if (!(defined $db_id)){
                       ...
                       if (defined $AoH_local_id[$level-1]{$hash_level_name{$level-1}}){
                          my $hash_id_key=$hash_level_name{$level-1}.":".$AoH_local_id[$level-1]{$hash_level_name{$level-1}};
                          if (defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $db_id){
                               print "\n4. you define two different record with same local_id:$AoH_local_id[$level-1]{$hash_level_name{$level-1}}: at line:$line";
	  	          }
                          $hash_id{$hash_id_key}=$db_id;
		       }
                    }
                    ...
                    if (!(defined $hash_db_id_deleted{$db_id_key})){
                       my $hash_id_key=$hash_level_name{$level-1}.":".$AoH_local_id[$level-1]{$hash_level_name{$level-1}};
                       $AoH_db_id[$level-1]{$hash_level_name{$level-1}}=$db_id;
                       if ( defined $AoH_local_id[$level-1]{$hash_level_name{$level-1}} && defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $db_id){
                              print "\n5. you define two different record with same local_id:$AoH_local_id[$level-1]{$hash_level_name{$level-1}}: at line:$line";
	 	       }
                       $hash_id{$hash_id_key}=$db_id;
                    }
                    else {
                      ...
                      if (!$db_id){
                         ...
                         if (defined $AoH_local_id[$level-1]{$hash_level_name{$level-1}}){
                            my $hash_id_key=$hash_level_name{$level-1}.":".$AoH_local_id[$level-1]{$hash_level_name{$level-1}};
                            if (defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $db_id){
                                 print "\n6. you define two different record with same local_id:$AoH_local_id[$level-1]{$hash_level_name{$level-1}}: at line:$line";
		            }
                           $hash_id{$hash_id_key}=$db_id;
		         }
                      }
	     ...
	     if ($db_id && defined $AoH_local_id[$level-1]{$hash_level_name{$level-1}}){
               my $hash_id_key=$hash_level_name{$level-1}.":".$AoH_local_id[$level-1]{$hash_level_name{$level-1}};
               if (defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $db_id){
                    print "\n7. you define two different record with same local_id:$AoH_local_id[$level-1]{$hash_level_name{$level-1}}: at line:$line";
	       }
               $hash_id{$hash_id_key}=$db_id;
  	     }
	     if ($db_id){
...
#    }
  ...
  #}
   ...
   #The parsers are not required to give you all of an element's character data in one chunk, The usual approach is to accumulate data in the character event and defer processing it until the end element event
   $character.=$data;
  ...
  my $hash_ref;
    ...
    # $character= utf8($character)->latin1;
     if ($element_name ne $root_element && defined $hash_ddl{$parent_element}){
        ...
        #if  (defined $hash_ref_cols->{$element_name} && ($character =~/\S/ ||  $character eq '-') && $character ne "\t"){

         if  (defined $hash_ref_cols->{$element_name} && defined $character && $character =~/.+/){
   ...
   if ($hash_ddl{$element_name}) {
        ...
        if (defined $hash_ref_cols->{$hash_level_name{$level+1}} && !$hash_ddl{$hash_level_name{$level+1}} && !(defined $AoH_ref[$level]{$hash_level_name{$level}})){
               ...
               if ($hash_level_op{$level} eq $OP_UPDATE){
                  ...
                  if ($validate_level == $validate_db) {
                       ...
                       if ($db_id && defined $AoH_local_id[$level]{$element_name}){
                            ...
                            if (defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $db_id){
                                 print "\n8. you define two different record with same local_id:$AoH_local_id[$level]{$element_name}: at line:$line";
		            }
                            $hash_id{$hash_id_key}=$db_id;
	               }
                       if ($db_id){
	      ...
	      if ($validate_level == $validate_db){
	            ...
	            if ($hash_id{$key} eq $db_id && $element_name eq $temp_table){
                        ...
                        if (defined $hash_db_id_deleted{$db_id_key}){
                            ...
                            if ($db_id){
                                ...
                                if (defined $AoH_local_id[$level]{$element_name}){
                                   my $hash_id_key=$element_name.":".$AoH_local_id[$level]{$element_name};
                                   if (defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $db_id){
                                       print "\n9. you define two different record with same local_id:$AoH_local_id[$level]{$element_name}: at line:$line";
	 	                   }
                                   $hash_id{$hash_id_key}=$db_id;
				}
                            }
                        ...
                        if ($db_id){
                            ...
                            &_create_log(\%hash_trans, $hash_ref , $element_name );
                                ...
                                if (defined $AoH_local_id[$level]{$element_name}){
                                   my $hash_id_key=$element_name.":".$AoH_local_id[$level]{$element_name};
                                   if (defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $db_id){
                                       print "\n10. you define two different record with same local_id:$AoH_local_id[$level]{$element_name}: at line:$line";
		                   }
                                   $hash_id{$hash_id_key}=$db_id;
				}
                        }
                 ...
                 if (!(defined $db_id)){
                     ...
                     if (defined $AoH_local_id[$level]{$element_name}){
                        my $hash_id_key=$element_name.":".$AoH_local_id[$level]{$element_name};
                        if (defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $db_id){
                           print "\n11. you define two different record with same local_id:$AoH_local_id[$level]{$element_name}: at line:$line";
	 	        }
                        $hash_id{$hash_id_key}=$db_id;
		     }
		 }
                     ...
                     if (defined $AoH_local_id[$level]{$element_name}){
                        my $hash_id_key=$element_name.":".$AoH_local_id[$level]{$element_name};
                        if (defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $db_id){
                           print "\n12. you define two different record with same local_id:$AoH_local_id[$level]{$element_name}: at line:$line";
		        }
                        $hash_id{$hash_id_key}=$db_id;
		     }
		 }
			 ...
			 if (defined $AoH_local_id[$level]{$element_name}) {
                             my  $hash_id_key=$element_name.":".$AoH_local_id[$level]{$element_name};
                             if (defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $db_id){
                                  print "\n13. you define two different record with same local_id:$AoH_local_id[$level]{$element_name}: at line:$line";
	 	             }
                             $hash_id{$hash_id_key}=$db_id;
			 }
		       }
		   	   ...
		   	   if (defined $AoH_local_id[$level]{$element_name}) {
                               my  $hash_id_key=$element_name.":".$AoH_local_id[$level]{$element_name};
                               if (defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $db_id){
                                 print "\n14. you define two different record with same local_id:$AoH_local_id[$level]{$element_name}: at line:$line";
		               }
                               $hash_id{$hash_id_key}=$db_id;
		  	   }
		         }
		...
		if ($db_id){
             ...
             if ($db_id && defined $AoH_local_id[$level]{$element_name} && $AoH_op[$level]{$element_name} ne 'delete'){
                 my $hash_id_key=$element_name.":".$AoH_local_id[$level]{$element_name};
                 #here to catch the incorrect macro:use same id for different record
                 if (defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $db_id){
                    print "\n16. you define two different record with same local_id:$AoH_local_id[$level]{$element_name}: at line:$line";
		 }
                 $hash_id{$hash_id_key}=$db_id;
        ...
        #for case using ref attribuate to ref object, self is still table_element. Key difference: use ref to retrieve $hash_data_ref
            ...
            if ($hash_level_op{$level} eq $OP_UPDATE){
               ...
               if ($validate_level == $validate_db){
                       ...
                       if ($db_id && defined $AoH_local_id[$level]{$element_name}){
                            my $hash_id_key=$element_name.":".$AoH_local_id[$level]{$element_name};
                            if (defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $db_id){
                              print "\n17. you define two different record with same local_id:$AoH_local_id[$level]{$element_name}: at line:$line";
		            }
                            $hash_id{$hash_id_key}=$db_id;
	               }
                       if ($db_id){
	      ...
	      if ($validate_level == $validate_db){
               ...
               &_create_log(\%hash_trans, $hash_ref, $element_name);
                 ...
                 if (!$db_id){
                     ...
                     if (defined $AoH_local_id[$level]{$element_name}){
                        my $hash_id_key=$element_name.":".$AoH_local_id[$level]{$element_name};
                        if (defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $db_id){
                           print "\n18. you define two different record with same local_id:$AoH_local_id[$level]{$element_name}: at line:$line";
		        }
                        $hash_id{$hash_id_key}=$db_id;
		     }
		 }
			 ...
			 if (defined $AoH_local_id[$level]{$element_name}) {
                             my  $hash_id_key=$element_name.":".$AoH_local_id[$level]{$element_name};
                             if (defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $db_id){
                                print "\n19. you define two different record with same local_id:$AoH_local_id[$level]{$element_name}: at line:$line";
		             }
                             $hash_id{$hash_id_key}=$db_id;
			 }
		       }
		   	   ...
		   	   if (defined $AoH_local_id[$level]{$element_name}) {
                               my  $hash_id_key=$element_name.":".$AoH_local_id[$level]{$element_name};
                               if (defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $db_id){
                                  print "\n20. you define two different record with same local_id:$AoH_local_id[$level]{$element_name}: at line:$line";
	  	               }
                               $hash_id{$hash_id_key}=$db_id;
		  	   }
		         }
		...
		if ($db_id){
            ...
            if ($db_id && defined $AoH_local_id[$level]{$element_name} && $AoH_op[$level]{$element_name} ne 'delete'){
               my $hash_id_key=$element_name.":".$AoH_local_id[$level]{$element_name};
               if (defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $db_id){
                    print "\n21. you define two different record with same local_id:$AoH_local_id[$level]{$element_name}: at line:$line";
	       }
               $hash_id{$hash_id_key}=$db_id;
	    }
            if ($db_id){
             ...
             if (defined $id){
                $AoH_data[$level]{$key}=$id;
                 if (defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $id){
                    print "\n22. you define two different record with same local_id:: at line:$line";
		 }
	  ...
	  if (defined $hash_id{$hash_id_key}){
             ...
             if ($id){
                $AoH_data_new[$level]{$key}=$id;
                 if (defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $id){
                    print "\n23. you define two different record with same local_id:: at line:$line";
		 }
                $hash_id{$hash_id_key}=$id;
	     }
             else {
	   ...
	   if (defined $hash_id{$hash_id_key}){
             ...
             if ($id){
                $AoH_data[$level]{$key}=$id;
                if (defined $hash_id{$hash_id_key} && $hash_id{$hash_id_key} != $id){
                    print "\n24. you define two different record with same local_id:: at line:$line";
		}
                $hash_id{$hash_id_key}=$id;
	     }
             else {
...
sub _data_check(){
	...
	if (exists $hash_unique_key{$temp[$i]} && $DELETE_BATCH !=1 && defined $AoH_op[$level]{$table} && $AoH_op[$level]{$table} ne $OP_DELETE){
...
sub xml_formatted($)            # Convert special characters to XML form.
{
    my $s = $_[0];
    chomp $s;
    $s =~ s/&/&amp;/g;
    ...
    $s =~ s/</&lt;/g;
    $s =~ s/>/&gt;/g;
    $s =~ s/(&amp;)([a-z]{1,2})(gr;)/$greek{$2}?"&#$greek{$2};":"$1$2$3"/egi;	# Graecum est.
    return $s;
}
...
sub _create_log(){
   ...
   print  "\nsorry, for some reason, this process stop before finish the following main transaction(child of root):";
   my $line=$parser->location()->{LineNumber};
