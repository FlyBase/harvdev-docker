...
my %hash_ddl;
# if there are triggers which may change the unique_key value, then retrieve id after insertion will be DB specific. Default to use triggers
my $TRIGGER=1;
...
sub new {
 ...
 my $user=undef;
 my $password=undef;
 my $ddl_property=undef;
 my $commit=undef;
      ($db_type, $database, $host, $port, $user, $password, $ddl_property, $commit) =
     XML::XORT::Util::GeneralUtil::Structure::rearrange(['db_type', 'db', 'host', 'port', 'user', 'password','ddl_property','commit'], @_);
...
$self->{'autocommit'}=$commit;
    ...
    my $pro=Properties->new($ddl_property);
...
sub _new {
   ...
   my $commit=$dbh_hash->{'autocommit'};
   $commit=$AUTOCOMMIT  if (!(defined $commit) || !( $commit ==1 ||$commit ==0));
   chomp($password);
$self->{'db_type'}=$db_type;
$self->{'database'}=$database;
$self->{'host'}=$host;
...
sub get_one_value(){
  ...
  my   $row_array = $query->fetchrow_arrayref;
    ...
    $query->finish();
    #$cache_sql_one_value->set($stm, $value);
    return $value;
}

=head2 _get_one_value

  Arg [1]    : $dbh
  Arg [2]    : $stm
  Example    :
  Description: private method to get one value
...
sub execute_sql(){
  ...
  my $stm=shift;
  my $query=$dbh->prepare($stm);
     $query->execute or die "Unable to execute query:$stm:\n$dbh->errstr:\n";
     $query->finish();
...
#    return $ref if (defined $ref);
    ...
    my $query=$dbh->prepare($stm);
    $query->execute or die "Unable to execute query: $dbh->errstr:$stm\n";
    my $hash_ref=$query->fetchrow_hashref;
    #$cache_sql_row_hashref->set($stm,$hash_ref );
    #$cache_sql_row_hashref{$stm}=$hash_ref;
    return $hash_ref;
}
...
#      $cache_sql_all_hashref->set($stm, \%hash_ref);
      return \%hash_ref;
    }
    else {
       #warn "\nno return here:\n$stm";
...
sub db_insert(){
  my $self=shift;
     ...
     if ($self->{'db_type'} eq "postgres") {
           $table_seq=&_get_one_value($self->{'dbh'},sprintf("SELECT seq.relname::text FROM pg_class src, pg_class seq,pg_attribute, pg_depend where pg_depend.refobjsubid = pg_attribute.attnum AND pg_depend.refobjid = src.oid AND  seq.oid = pg_depend.objid AND pg_attribute.attrelid = src.oid AND src.relname = '%s' and pg_attribute.attname='%s'", $table, $id));
           $stm_select_seq="select last_value from $table_seq";
     }
     elsif ($self->{'db_type'} eq "mysql"){
        $stm_select_seq="select last_insert_value()";
     }
     else {
        $table_seq=$table."_".$id."_seq";
        $stm_select_seq="select last_value from $table_seq";
     }
    ...
    if ($stm_insert && $stm_select && $stm_update){
          ...
          if ($db_id){
            ...
            foreach my $key (keys %$ref_temp1){
                #attention: +1 and 1 should see as 'same', also \\, boolean 1 and t/true,
              $ref_temp1->{$key}=~s/\\/\\\\/g if (defined $ref_temp1->{$key});
	      if (defined  $ref_temp1->{$key} && $ref_temp1->{$key} =~/^\d\d*/){#here for digital value comparison
              ...
              elsif (defined  $ref_temp1->{$key}) {#here is for string value comparison
              ...
              elsif (!(defined  $ref_temp1->{$key}) && defined $data_hash_ref->{$key}){
                  $update_need='true';
                  print "\nneed update for this column:$key, old value is null, new value:$data_hash_ref->{$key}\nstm_select:$stm_select_temp\n\n$stm_update\n";
                 last;
	      }
	    }
            $query_temp->finish();
...
sub _data_type_checker(){
  ...
  #  print "\ndata_type:$data_type";
   ...
   #here to deal with special case: O'Reiley, 
    foreach my $key (keys %$hash_ref){
        $hash_ref->{$key} =~ s/\'/\'\'/g if (defined $hash_ref->{$key} );
        #$hash_ref->{$key} =~ s/\\/\\\\/g;#already done in XMLParser.pm
    }
   ...
   # print "\n\ndata_type:$data_type";
	...
	if ($temp1[1] !~ /int|serial|float|smallint|integer|bigint|decimal|numeric|real|bigserial/ && (!(defined $hash_ref->{$temp1[0]}) || $hash_ref->{$temp1[0]} ne 'NULL' )){
            if (defined $temp1[1] && $temp1[1] =~  /boolean/ && defined $hash_ref->{$temp1[0]} && defined  $hash_boolean{$hash_ref->{$temp1[0]}} && defined($hash_ref->{$temp1[0]}) && ($hash_ref->{$temp1[0]} !~ /^\'\S*\'$/)){
            ...
            if (defined $temp1[1] && $temp1[1] =~/boolean/ && defined $hash_ref->{$temp1[0]} &&  exists $hash_boolean{$hash_ref->{$temp1[0]}} ){
               $hash_ref->{$temp1[0]}=$hash_boolean{$hash_ref->{$temp1[0]}};
	    }
    }
...
sub create_log(){
   ...
   my $file=shift;
    my $fh = new IO::File ">:utf8 $file";
    if (defined $fh) {
       foreach my  $key (keys %$hash_local_id){
          print $fh "$key\t$hash_local_id->{$key}\n";
       }
       $fh->close;
    }

   print "\nsorry, for some reason, this process stop before finish the following transaction:";
