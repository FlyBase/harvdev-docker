...
XML::XORT::Dumper::DumpXML - Generic Dumper module to dump any database into XORT format XML file.

=head1 SYNOPSIS

    my $dump_obj=XML::XORT::Dumper::DumperXML->new('chado');
         $dump_obj->Generate_XML(-file=>$file_chado,
...
=cut
 ...
 package XML::XORT::Dumper::DumperXML;
 ...
 use XML::DOM;
 use Encode;
 # use encoding 'utf8'; # DEPRECATED - encoding pragma no longer supported in modern Perl
 # UTF-8 handling is done via Encode module and PERL_UNICODE environment variable
 use strict;
 #use Cache::FileCache;
...
sub Generate_XML {
   ...
   my ( $table, $file,  $struct_type, $op_type, $format_type, $dump_spec, $ref_obj, $dumpspec_data,$ddl_file, $cache_conf, $entity_dec) =
   XML::XORT::Util::GeneralUtil::Structure::rearrange(['tables', 'file', 'struct_type', 'op_type', 'format_type', 'dump_spec', 'loadable', 'app_data','ddl_property','cache_conf','entity_declaration'], @_);
   my $stm;
   ...
   system("rm $file") if (defined $file && -e $file);
#   $file=">>".$file;
#   open (LOG, $file) or die "could not write to file:$file";
    #here if NO output file specified, default to STDOUT
    if (defined($file) && $file =~/\w+/) {
	#open FILEH, ">:utf-8", $file;
        #open FILEH, ">:utf-8", $file;
      # use open ':utf8'; # input and output default layer will be UTF-8
       open FILEH, ">$file";
       #$FILEH=new IO::File ">$file";
	select FILEH;
     }
     else {
        binmode(STDOUT, ":utf8");
 ...
 #load the propertity information and open the connection with database
   ...
   print  "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<!-- created by Pinglei Zhou, Flybase Harvard University -->\n<!--created from Database:$database using dump_spec:$dump_spec at ".localtime()." -->\n";
   #if ($ENTITY_DECLARATION ==1){
   print <<'NOGROK';
<!DOCTYPE FBchado [
<!ENTITY lt "&#38;#60;">
<!ENTITY gt "&#62;">
...
# }
         ...
         if ($node_type eq ELEMENT_NODE && defined $hash_ddl{$node_name} && !(defined $hash_tables_pseudo{$node_name})){
               ...
               if (defined $query_all && (($query_all =~/ group / && $query_all=~/by/) || ($query_all =~/ order / && $query_all=~/by/)) ){
                   ...
                   for my $n2 (0..$#{$ref_all_array}){
                        ...
                        &_table2xml($ref_array, "\t", $node_name, $op_type, $format_type, $MODULE,  $dbh, $ref_obj, $node,\@cols);
		    ...
		    if ($s=~/\.$table_id/ || $s=~/distinct/){
		      ...
		      if (!( @cols)){
                      ...
                      #print encode('utf-8', $xml) if ($DEBUG !=2);
                      #undef $xml;
                      &_table2xml($ref_join_array->[0], "\t", $node_name, $op_type, $format_type, $MODULE,  $dbh, $ref_obj, $node,\@cols);
                  }
                  $sth1->finish();
                  $sth->finish();
               }
   ...
   #no dump spec, then use default;
    ...
    for my $i(0..$#array_tables) {
       my $string_primary_key=$array_tables[$i].'_primary_key';
       my $table_id=$hash_ddl{$string_primary_key};
       $stm=sprintf("select $table_id from $array_tables[$i]");
            print "\n$stm;" if ($DEBUG==2);
         my $sth01=$dbh->prepare($stm);warn "\n$stm" if ($DEBUG==1);
            $sth01->execute or die "Unable to execute query: $dbh->errstr:$stm\n";
         my $ref_all_array01=$sth01->fetchall_arrayref;
             my $sth02=$dbh->prepare(sprintf ("select * from $array_tables[$i] where $table_id=?"));
         for ( my $n3=0;$n3<=$#{$ref_all_array01}; $n3++){
                $sth02->bind_param(1, $ref_all_array01->[$n3][0]);
                $sth02->execute;
                my $ref_row_array02=$sth02->fetchrow_arrayref;
                my @cols;
                for my $o (0..$sth02->{NUM_OF_FIELDS}-1){
                       push @cols, $sth02->{NAME}->[$o];
                }
              #my $xml=&_table2xml($ref_row_array02, "\t", $array_tables[$i], $op_type, $format_type, $struct_type,  $dbh, undef, undef, \@cols);
              #print encode('utf-8', $xml);
              &_table2xml($ref_row_array02, "\t", $array_tables[$i], $op_type, $format_type, $struct_type,  $dbh, undef, undef, \@cols);
	  }
           $sth02->finish();
           $sth01->finish();
       }
...
#=cut
  ...
  foreach my $i (0..$#{$array_ref}){
       my $data=$array_ref->[$i];
       if (defined $data){
                       #This should be done in _de_encoding() to avoid turning '&agr;' into '&amp;agr;'
                       #$data =~ s/\&/\&amp;/g;
                        #$data =~ s/</\&lt;/g;
                        #$data =~ s/>/\&gt;/g;
                        #$data =~ s/\"/\&quot;/g;
                        #$data =~ s/\'/\&apos;/g;
            #1 use entity declaration for Greek chararcter, 0 write out raw data in UTF-8 (default), 2: use plain text,
	    if ($ENTITY_DECLARATION ==1){
                        $data=&_raw_to_entity($data);
                        $data=&_de_encoding($data);
	     }
             elsif ($ENTITY_DECLARATION==2){
                        $data=&_raw_to_entity($data);
                        $data=&_decon($data); 
                        $data=&_de_encoding($data);
             }
 ...
 if ($format_type eq $LOCAL_ID){
    if ($op_type ne ''){
        print encode('utf-8', "\n$indent<".$table." id=\"".$id."\" op=\"".$op_type."\">" ) if ($DEBUG !=2);
 
    }
    else {
        print encode('utf-8',"\n$indent<".$table." id=\"".$id."\">") if ($DEBUG!=2);
    }
 }
 else {
    if ($op_type ne ''){
        print encode('utf-8',"\n$indent<".$table." op=\"".$op_type."\">") if ($DEBUG!=2);
    }
    else {
   ...
   if (defined $hash_ddl{$foreign_table_ref}){
	 ...
	 if (defined $node){
	  ...
	  if (@array_table_obj_cols==0){
         ...
         $array_ref->[$i]=undef;undef $data_sub;
...
#    $xml=$xml.$xml_sub;
 ...
 #$xml=$xml."\n$indent</$table>";
 print encode('utf-8', "\n$indent</$table>") if ($DEBUG !=2);undef $xml;
 return $xml;
}
...
#warn "\n$sgml";#warn "\nequal here:$sgml:chr(246):" and exit(1) if ($sgml eq chr(246));
  	#$sgml=&decode_utf8($sgml,Encode::FB_CROAK); ###decode first, where is this method ? from Encode ?
         $sgml=utf8::is_utf8($sgml) ? $sgml : &decode_utf8($sgml, Encode::FB_CROAK);
my $agrave=chr(224);
my $Agr=chr(913);
my $Bgr=chr(914);
my $Ggr=chr(915);
