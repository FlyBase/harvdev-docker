...
my $ddl_properties_file=">".$conf."/ddl.properties";
...
while (<IN>){
 ...
 # end of create table, print out all result
 elsif ($input =~/\)\s*\;/ && $input !~/index|constraint|add|sequence|grant|alter/){
   my $table_col;
   my $tale_data_type;
  ...
  undef  %hash_non_null_default;
   ...
   undef $table_data_type;
   undef $non_null_cols;
   undef $non_null_default;
   $start=0;
 }

  # here do all the parsing ....
 if ($start==1){
   if (($input !~/unique|primary|foreign|create|table|check/) || ($input =~/uniquename/ && $input =~/text/) ){
...
while (<IN>){
 ...
 # end of create table, print out all result
 elsif ($input =~/\)\s*\;/ && $input !~/index|constraint|add|sequence|grant|alter|CONSTRAINT|ADD|INDEX|SEQUENCE|GRANT|ALTER/){
   my $table_col;
   my $tale_data_type;
   foreach my $key (keys %hash_table){
     my $value=$hash_table{$key};
...
sub usage()
 {
  print "\nusage:$0 [-d ddl file] [-v view and function file]",
    ...
    "\n\nexample1: $0  -d  ../conf/chado.ddl -v ../conf/function_view.ddl\"\n",
