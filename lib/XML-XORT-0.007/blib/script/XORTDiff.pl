...
use XML::DOM;
use XML::XORT::XMLDiff::XORTTreeDiff;
use Getopt::Std;
...
my $obj=XML::XORT::XMLDiff::XORTTreeDiff->new($opt{f}, $opt{s});
...
if ($opt{p} eq $DIFF_PATERN_DEEP){
    ...
    $obj=XML::XORT::XMLDiff::XORTTreeDiff->new($opt{s}, $opt{f});
...
sub usage()
 {
  print "\nusage: $0 [-f first_file] [-s second_file] [-i is_recovery]",
    ...
    "\n -l level        : all/ref/custom, all: check all cols of object, ref:only check object identity,",
    "\n                   custom:user decide which table/cols will check, configure ./conf/XORTTreeDiffConfig.xml",
