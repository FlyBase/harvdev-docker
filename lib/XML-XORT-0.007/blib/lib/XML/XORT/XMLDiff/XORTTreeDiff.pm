package XML::XORT::XMLDiff::XORTTreeDiff;
use strict;
use vars qw/$VERSION/;
...
#this number will be incremental to represent different object if need
my $random_number=0;
...
my $XORTTreeDiffConfig_file=$conf."/XORTTreeDiffConfig.xml";
...
#hash to store all random numbers to make sure NOT assign same random for different object
my %hash_random;
       ...
       if ($node1_type ==ELEMENT_NODE && $node1_name eq $EXCLUDE_TABLE){
          ...
          for my $j(1..$nodes2->getLength()){
            ...
            if ($node2_type ==ELEMENT_NODE && $node2_name eq 'table'){
               ...
               print "\nnot consider those table:$table_name" if ($DEBUG==1);
            }
          }
	    ...
	    if (defined $child_node_1){
	        ...
	        if (defined $child_node_2){
                  ...
                  if ($equal_temp ==$NODE_EQUAL && ($child_node_1->getNodeType ==ELEMENT_NODE)){
                    ...
                    $random_number++;
                    my $random1=$random_number;
                    $random_number++;
                    my $random2=$random_number;
                    $hash_random{$random1}=1;
                    $hash_random{$random2}=1;
  ...
  if ($node1->getNodeType ==TEXT_NODE && $node2->getNodeType ==TEXT_NODE){
     ...
     if ($data1 eq $data2 ||($data1 !~/\w/ && $data2 !~ /\w/)){
		   ...
		   if (defined $hash_ddl{$node2->getNodeName()}){
			   ...
			   if ($match_flag1=~/\w/){
                     ...
                     if ($match_link_table ==1){
                        ...
                        $random_number++;
                        my $random3=$random_number;
                        $random_number++;
                        my $random4=$random_number;

                        $hash_random{$random3}=1;
                        $hash_random{$random4}=1;
	  ...
	  if ($equal_f ==$NODE_EQUAL  && $equal_r ==$NODE_EQUAL){
                ...
                $random_number++;
                my $random5=$random_number;
                $hash_random{$random5}=1;
                ...
                my $random5=$random_number;
                $random_number++;
                my $random6=$random_number;
                $hash_random{$random5}=1;
                $hash_random{$random6}=1;
                $node1->setAttribute ($EQUAL_ATTRIBUTE, $random5);
