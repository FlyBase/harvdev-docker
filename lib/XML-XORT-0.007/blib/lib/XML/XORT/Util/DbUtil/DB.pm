...
sub _new {
   ...
   $commit=$AUTOCOMMIT && warn "use default value:$AUTOCOMMIT" if (!(defined $commit) || !( $commit ==1 ||$commit ==0));
   chomp($password);
$self->{'db_type'}=$db_type;
$self->{'database'}=$database;
$self->{'host'}=$host;
  ...
  if ($data_update_list){
   ...
   print "\ndb_force: stm_select:$stm_select\nstm_insert:$stm_insert\nstm_update:$stm_update" if ($DEBUG==1);
    ...
    if ($stm_insert && $stm_select && $stm_update){
	      ...
	      if (defined  $ref_temp1->{$key} && $ref_temp1->{$key} =~/^\d\d*/){
